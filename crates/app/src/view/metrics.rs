...
/// What a card's name and subtitle take out of its cell.
///
/// **An estimate, not a port.** `LibraryPage.qml` gave the `CoverArt` inside
/// the card `Layout.fillHeight: true`, so the cover took whatever the labels
/// did not — the QML never names a number. This is a name and a subtitle at
/// the sizes below, their spacing, and nothing else, and it moves the moment
/// anyone puts a real measurement against it.
pub const CARD_CHROME_HEIGHT: f32 = 70.0;
