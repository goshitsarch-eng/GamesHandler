...
/// SHA-256, for the fixture-integrity check above.
///
/// Written out rather than pulled in: this is the only place the crate needs a
/// hash, and `crates/core/Cargo.toml`'s dependency list being exactly
/// `serde` + `serde_json` is a design rule, not an accident (D-03). The
/// implementation is the textbook one, and it is checked against the published
/// digests for the empty string and `"abc"` before it is used.
