...
| PKG-06 | P3 | **The metainfo has no screenshots and no keywords.** Both are optional in the AppStream specification, which is why the validator passes — but a screenshot is required for an app to be presented properly in a software centre, and keywords are what AppStream search matches on. The `.desktop` file has keywords; the metainfo does not, so the two disagree about how the app is described. | `grep -n "<screenshots\|<screenshot\|<keywords" data/com.goshapps.GameHandler.metainfo.xml` → **no matches**. The desktop file does carry them: `data/com.goshapps.GameHandler.desktop:10` `Keywords=Wine;Proton;Windows;Games;Emulation;`. | Reduced discoverability in software centres and a listing that renders without imagery. The validator is clean, so nothing will prompt anyone to add these. | Add one or two `<screenshot type="default">` entries pointing at images hosted with the project (they must be reachable over HTTPS — not verified here) and a `<keywords>` block mirroring the desktop file's. **Status: PARTIAL.** **Keywords: FIXED.** The metainfo now carries `<keywords>` (`data/com.goshapps.GameHandler.metainfo.xml:42-55`): the five words the desktop entry advertises, lowered because AppStream requires lowercase, plus `game launcher` and `game manager`, two words a software-centre search would plausibly use and that the desktop list does not have. `appstreamcli validate --no-net --pedantic` is unchanged: still exit 0, still the one pre-existing `cid-contains-uppercase-letter` pedantic note that is inherent to the app id. Regression test `test_metainfo_keywords_cover_the_desktop_entrys` compares the two files' lists rather than asserting a literal, so the metainfo may add a word but never silently lose one the desktop entry advertises; mutation-verified by deleting `emulation` from the metainfo, which fails it on the difference. **Screenshots: NOT FIXED, and not fixable from here.** The repository contains no screenshot — the only PNGs are the five app icons — and an AppStream `<screenshot>` must be a remote URL, so adding one today would mean either fabricating a record pointing at an image that does not exist or committing a screenshot that has not been taken. The test that settles it: run the app under a compositor, capture the Library and the Runners page, host them, then add `<screenshots><screenshot type="default"><image>…</image>`. This is the one PKG row whose remaining half is an external dependency rather than unfinished work, and it is recorded as such rather than left looking like a backlog item. ||
...
| PKG-08 | P3 | **A comment in `data/meson.build` asserts a packaging regression that does not exist, and its cited evidence is falsified by the manifest.** The comment states that the LICENSE install "has no counterpart in the Rust manifest, which installs only the osslsigncode and DXVK licence texts", asserts that `grep LICENSE` over the manifest "finds no source that is the application's own COPYING, and no `"LICENSE"` path at all", and concludes "This is a live regression, not a hypothetical". The manifest installs the application's own LICENSE at line 94, and the built tree contains it byte-identical. | The stale claim: `data/meson.build:13-23` (the quoted sentences are at `:14-17` and `:18`). The counter-evidence: `build-aux/flatpak/com.goshapps.GameHandler.json:94` — `"install -Dm0644 LICENSE ${FLATPAK_DEST}/share/licenses/com.goshapps.GameHandler/LICENSE"`; and the built tree, `ls -la build-flatpak/files/share/licenses/com.goshapps.GameHandler/` → `LICENSE` (35,149 bytes), `DXVK.txt` (1,074), `osslsigncode-LICENSE.txt` (1,530); `cmp LICENSE build-flatpak/files/share/licenses/com.goshapps.GameHandler/LICENSE` → **byte-identical**. The same comment's other citation, `…json:91-93` for the three metadata installs, is still accurate (`:91-93` are exactly those three `install -Dm0644` lines), which is what makes the stale half easy to believe. | Anyone reading this file to decide whether the licence is shipped is told, with an emphatic and specific claim of verification, the opposite of the truth. The cost is asymmetric: believing it leads to "fixing" something that is already correct — or, worse, to deleting the meson `install_data` on the strength of a comment that is wrong about which direction the gap runs. | Delete or correct `data/meson.build:13-23`. The file is the Python tree's and the whole block could go with it when that tree is retired (see PKG-02), but while it stays it should not assert something a one-line grep contradicts. **Status: FIXED.** The comment is corrected rather than deleted, and **three of its claims were false, not one**. (a) The central one: the Rust manifest **does** have a counterpart for the LICENSE install — `build-aux/flatpak/com.goshapps.GameHandler.json:101`, `install -Dm0644 LICENSE ${FLATPAK_DEST}/share/licenses/com.goshapps.GameHandler/LICENSE`. (b) So the cited evidence is false as stated: `grep LICENSE` over the manifest finds that line, which is the application's own `COPYING`, and there **is** a `"LICENSE"` path in the manifest. (c) The manifest line numbers the comment cited for the shared files (91-93) are now 94-96, after the cargo build commands were added above them. (d) The asserted consequence — "removing this file without adding an equivalent line to the manifest would drop the application's own licence from the Flatpak" — is inverted: **the manifest line is what installs it now**, so retiring this file would change nothing about the shipped licence. Measured, not read: `build-flatpak/files/share/licenses/com.goshapps.GameHandler/LICENSE` exists in the built tree and is byte-identical to the repository's `LICENSE` (`sha256sum` agrees on both, 35,149 bytes, GPL-3.0). The regression was real when it was written and was closed by `8abac0b` (PKG-01), which added the line this comment says is missing — so the comment describes the pre-fix manifest, which is why the row's own evidence is stale rather than invented. Recorded here rather than silently overwritten because this comment is the kind of record a maintainer acts on. ||
| PKG-09 | P3 | **`flatpak-contents` and `cargo-sources` both shell out to a bare `python3` for their comparison helpers, independently of the interpreter chosen to run the generator.** The generator is run with a discovered interpreter (`$py`, the one that can `import aiohttp`), but the two inline comparison scripts are invoked with `python3` from `PATH`. On a machine where the aiohttp-capable interpreter is not the default one, the generator runs under one Python and the comparisons under another. | `scripts/verify.sh:1606` (`python3 -c "$COMPARE_PY" …`), `:1534` (the git-coverage helper), against `$py` usage at `:1598`; `find_generator_python` at `:1440-1450` is what selects `$py`. | Low: both helpers are pure `json` and `re` over strings, so a version skew is unlikely to change a result today. It is a latent trap — the day a helper uses a feature that differs, the failure will look like a stale `cargo-sources.json` rather than a wrong interpreter. | Use `"$py"` for the helpers too, or assert that `python3` and `$py` are the same interpreter at stage entry. |

| PKG-11 | P2 | **The verification chain was red at `fd7cf01`, and a red stage truncates every stage after it.** `scripts/verify.sh` runs `doc` fourth; it failed with four unresolved intra-doc links, and the eleven stages behind it — `test`, `cli`, `oracle-freshness`, `python-tests`, `cargo-lock`, `plan-counts`, `cargo-sources`, `cargo-sources-fresh`, `flatpak-build`, `smoke-test`, `desktop-metainfo`, `flatpak-contents` — **did not run**. The gate's own summary says so in the words the brief's standard asks for ("these are neither passes nor skips: nothing was verified about them"), which is the only reason this is visible at all: a report that had counted the un-run stages as passes would have read as a green tree. | Measured on a clean worktree at `fd7cf01`: `RUSTDOCFLAGS="-D rustdoc::broken_intra_doc_links" cargo doc --workspace --no-deps` exits 1 with four `error: unresolved link to …` — `Accessible::a11y_nodes` (`crates/app/src/view/a11y.rs:374`, a method on `Widget`, not on the app's own `Accessible` wrapper), `InstallError` (`crates/app/src/view/plugins.rs:148`, whose target is `gamehandler_core::plugins::InstallError` and whose link definition at `:33` is a *reference* definition, which does not make the name resolve in this file), `an_icons_picture_is_inset_inside_the_plate_by_the_icon_inset` (`crates/app/src/view/widgets.rs:973`, a `#[test]` in a `#[cfg(test)]` module, so rustdoc never sees it) and `shortcuts::search_the_framework_cannot_reach` (`crates/app/src/main.rs:628`, a private `fn` in a private module). `bash scripts/verify.sh` at the same revision: `passed: build fmt clippy`, `failed: doc`, then the `did not run` list. | The chain the brief's Definition of Done names did not complete, and nothing said so except the summary line. Anything a later revision broke in the twelve stages behind `doc` would have been invisible, and the four broken links were themselves invisible — a broken intra-doc link degrades to an unlinked code span, so the rendered page reads correctly and only the author's intent is lost. | Three of the four are links to items rustdoc cannot see from where they are written (a `#[cfg(test)]` test, a private module's private `fn`, the core crate's error type), so the fix is to write them as code spans rather than links, or to name the full path; the fourth is a name that does not exist on the type it names. **Status: FIXED.** All four are repaired — two re-pointed (`InstallError` to its full path, whose reference-style definition at `:33` did not help) and two demoted to code spans, because a link that cannot resolve is not a link — and the gate was then re-run: `doc` passes, and the twelve stages behind it are reached for the first time since the stage landed in `0289ffe`. **Reached is not the same as passed, and the first draft of this row claimed the gate "was then run end to end" — which is false.** The re-run gives `passed: build fmt clippy doc test cli oracle-freshness python-tests cargo-lock plan-counts cargo-sources`; `flatpak-build` fails and truncates `smoke-test`, `desktop-metainfo` and `flatpak-contents`, and `cargo-sources-fresh` skips for a missing prerequisite. So seven of the twelve ran green (`test`, `cli`, `oracle-freshness`, `python-tests`, `cargo-lock`, `plan-counts`, `cargo-sources`), one failed, three never ran and one skipped. The distinction is the whole point of the finding: a stage that was reached and a stage that passed are different facts, and the first version of this sentence is the same shape as the defect the row records. The four are recorded in the row rather than only in the diff because the *reason* each failed is the part that would otherwise be re-introduced. |
...
8. **The declared installs and the built tree agree, and the completeness check
   is real.** `verify.sh`'s `FLATPAK_CONTENTS` (`:1864-1874`) names nine
   non-binary installs, and the manifest declares each with the exact source and
   destination (`install -Dm0644` at `…json:94`, `:95`, `:96`, `:97`, `:98`,
   `:99`, `:100`, `:101`, `:102`; the binary at `:93` is exempted at
   `verify.sh:1830`). `verify.sh` does not merely compare the list against
   itself: `DECLARES_PY` (`:1795-1816`) parses the manifest's `build-commands`
   and requires the command to begin with `install ` and the token before the
   destination to equal the source, and `COMPLETENESS_PY` (`:1825-1859`) fails
   if the manifest installs something the list omits. Against the built tree the
   stage goes further and `cmp`s each installed file against its repo original
   (`:2170-2186`). I confirmed the end state independently (item 4). This is a
   well-built check; the gap the audit recorded in it was what it did *not* run
   (PKG-04, since fixed), not what it asserts.
9. **The `desktop-metainfo` stage is honest about being unable to run.** It runs
   ...
   otherwise (`verify.sh:1687-1708`), and — importantly — distinguishes a
   validator **verdict** from a **missing validator**: a failed verdict returns
   1 (`:1713`), while an absent tool returns `99` (`:1714`), which maps to an
   unrequested skip and **exit 3** (`:2224-2230`, `:2643-2649`). So "the
   validator was not installed" can never be reported as a pass. This is the
   correct handling of a prerequisite, and it is the pattern PKG-03 and PKG-07's
   missing `appstream-util` land in.
10. **The `banner_check` limit is documented rather than hidden.** The check
    ...
    which is what it says, and nothing more" (`verify.sh:222-227`). A partial
...
12. **`smoke-test.sh`'s exit-code contract is well-designed.** `0` = every check
    ...
    `smoke-test.sh:444-453` and translated by `verify.sh:1654-1657`. The
...
1. **Whether the exported bundle installs and runs.** `flatpak build-bundle`
   ...
   install it into a clean installation and run it. This is **not** the
   sandbox-execution gap PKG-04 described: that finding was that `verify.sh`
   never executed a built binary at all, and it is `FIXED` — the pipeline now
   runs `--version`/`--list` against the built tree and asserts the output
   shape. Executing a binary from the *build tree* is a strictly weaker
   property than installing the exported *bundle* into a clean installation and
   starting the GUI under a compositor, which remains untested here. The test
   that settles it: `flatpak
...
7. **Whether the Flatpak build succeeds right now from a clean state.** A
   ...
   `FLATPAK_LOCK="$ROOT/target/verify-flatpak.lock"` (`:706`) and uses
   `--force-clean` (`:1618`), so it is safe to run deliberately — it just was not
   run here.
