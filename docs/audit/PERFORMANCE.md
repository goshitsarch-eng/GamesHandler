...
read-only and every number in a finding's **Evidence** column is the pre-fix
figure taken at that commit. Where a row carries a **Status** tail, that tail's
numbers are a **post-fix** measurement of the changed tree — necessarily taken
later, against a different binary — and the two are labelled as such in the text
so a reader is never comparing a before with a before. Each row carries its own
...
|---|---|---|---|---|---|
| PERF-01 | P1 | **Every game's cover is classified on every frame — two filesystem syscalls per game with a cover, per frame — regardless of whether the tile is visible.** `CoverSource::classify` does `Path::new(cover_path).is_file()` (a `statx`) and then, when the file exists, `has_ico_magic` → `File::open` + `read_exact(&mut magic)` (an `openat` + `read`). It is called from `cover_plan` and again from `framed`, i.e. the work is done twice per rendered cover. Nothing culls it: the cull in the renderer (`if !_clip_bounds.intersects(&physical_bounds) { return; }`) happens at *draw* time, long after `view()` has run. | `crates/app/src/view/cover.rs:112-123` (`classify`, the `is_file()` early return at `:115`) and `:192-198` (`has_ico_magic`); `crates/app/src/view/widgets.rs:256` (`cover_plan`, defined `:255`) and `:688` (`framed`, defined `:687`); the cull is `iced/tiny_skia/src/engine.rs:585-588`. **Measured**, `strace -f -tt -e trace=%file` over a 3.572 s window of 100 forced redraws with 333 covers on disk: **6,624 cover-file syscalls (2,664 `statx` + 3,960 `openat`), and all 333 distinct cover paths were touched** — every cover, every frame, in a 1200×800 window that shows a few dozen tiles. With the cover files absent (the `Plate` path returns before the open) the same harness measured **9,990 `statx`, exactly 333 per frame across 30 frames**. | This is the dominant per-frame I/O cost and it scales linearly with library size, not with what is on screen. A 2,000-game library pays ~2,000 `statx` + ~2,000 `openat` per frame — on a software renderer that is already spending ~5–25 ms of CPU per redraw, so a large library spends a meaningful fraction of every frame in the kernel resolving files whose answer cannot have changed since the last frame. This is the same defect class the project already documents for the runner label (see PERF-06): the value is a pure function of data that only changes when the library is re-read. | Resolve the cover kind **once**, where the rows are built, and carry it as data — exactly the fix `crates/app/src/view/library.rs:253-271` already prescribes for the runner label ("a `State` field holding the resolved rows, updated where the library is re-read"). `CoverPlan` is already a value type with its own tests (`widgets.rs:240-259`), so it can be computed at load and stored alongside the game with no change to what is drawn. Failing that, at minimum compute it once per rendered cover per frame instead of twice. **Status: FIXED `be31a7b`.** The first of the two prescribed fixes, chosen over the second because the second leaves the cost proportional to what is drawn rather than to what exists. `crates/app/src/view/cover_cache.rs`'s `CoverCache::kind` memoises `CoverSource::classify` by cover path, and the cache is owned by `State` and threaded through every builder that draws a cover, so `view()` no longer reaches the filesystem at all. Verified against the real binary — 500 games, 333 covers, 100 forced redraws, counting only syscalls whose path contains `covers/`: **27,656 cover-path syscalls before (11,322 `statx` + 16,334 `openat`), 96 after (32 + 64)** — 288× lower, and the POST figure is a single startup burst rather than a per-frame recurrence. `strace` over the unit test alone agrees: 150 `statx`/156 `openat` before, 3/9 after. The uncached `CoverSource::classify` is left untouched in `crates/app/src/view/cover.rs:112` — the same file and line this row's evidence column names — and remains the authority the tests use as a control. The figures above are an independent re-run by the lead against both binaries built side by side from `f278b7d` and from the fix (the numbers differ from the first run's by the ~20 % run-to-run spread this file's *Measurement environment* paragraph already records; the direction and the order of magnitude do not). |
| PERF-02 | P1 | **Cover images are decoded at full source resolution and retained, and the cache is bounded only by "what was drawn in the last frame" — which, with no virtualization (PERF-03), is every game in the library.** The decoded pixmap is stored as `vec![0u32; image.width() * image.height()]`, i.e. 4 bytes per pixel at the *source image's own* dimensions, with no downscale-on-load; a separately resampled copy is cached under `(id, target_w, target_h)`, also at 4 bytes per pixel, and **both** are kept. The per-frame trim retains only entries hit in that frame, so nothing is evicted while it is still being drawn. | Cache and allocation: `iced/tiny_skia/src/raster.rs:123-128` (`entries: FxHashMap<raster::Id, Option<Entry>>` + `resampled`), `:151-171` (the native-resolution `vec![0u32; w*h]`), `:185-219` (`allocate_resampled`, a second full-size buffer), `:232-237` (`trim` retains only `hits`); the trim runs once per frame at `iced/tiny_skia/src/lib.rs:198`. **Measured**, RSS of the 500-game library read from `/proc/<pid>/status` `VmRSS` after the window had drawn, covers written as real JPEGs at each size: **19,860 kB with no covers** (figure supplied by the coordinator's measurement of the sandboxed app; my own no-cover runs were in the same ~20 MB band); **144,564 kB with 333 covers at 200×300**; **771,656 kB with 333 covers at 600×900**. The last is the decisive one: 333 × 600 × 900 × 4 bytes = **719 MB**, against an observed increase of 753 MB − 20 MB = **733 MB** — so essentially every one of the 333 covers is decoded and resident, not merely the few dozen the window can show. | Memory grows as `(native pixels × games)` with no cap. The 600×900 figures are not a stress test: 600×900 is an ordinary cover, and 753 MB of RSS for a 500-game library is enough to get the app OOM-killed on a small machine or a Steam Deck. A library of 500 games with 1000×1500 covers would be ~3 GB, and the app holds **both** the native and the resampled copy. | Downscale on load to the largest size the UI can draw (the grid cell is 200 px wide per `LibraryPage.qml:139`, cited at `crates/app/src/view/metrics.rs:40` as `GRID_CELL = (200.0, 300.0)`) and store only that, so `entries` never holds a full-resolution pixmap; and/or bound the cache by total bytes rather than by "hit last frame", since the last-frame rule is exactly what makes residency equal to library size here. Any fix should be measured with the method above — real cover files at a known size, `VmRSS` after a draw. **Status: FIXED `be31a7b`.** Both prescribed halves, because either alone still leaves the reported failure: the cache now decodes covers itself through `image` 0.25.10 with the longest axis capped at 512 (`cover_cache.rs`, `DECODE_MAX` — the reference's own number, `CoverArt.qml:68-69`'s `sourceSize`) and holds the result under a **64 MiB** byte budget with LRU eviction, so residency is bounded by the budget rather than by the library. Decoding here rather than handing iced a path is the part that had to move: iced's raster cache retains at the *source* image's own dimensions and its `Pipeline::load`/`allocate` are private, so there is no seam to hand it a bound — the handle is minted once and reused, which is also what makes iced's own cache a hit across frames. Measured against the real binary, 500 games / 333 covers, `VmRSS` sampled per second with one forced redraw per second: **peak 775,232 kB before, 90,640 kB after** (685,168 kB lower, 7.6×), against a no-cover baseline of 70,848 → 43,500 kB. The remaining ~46 MB above baseline is the documented budget. **One limit this fix does not remove, stated rather than implied:** the budget arithmetic is a bound and the measurement is at 500 games, not 10,000 — the cache is bounded by construction, but no run here demonstrates that at scale. Behaviour when a cover file is rewritten by something other than the app is also unchanged; the module docs and a test record it. |
| PERF-03 | P1 | **Nothing is virtualized: every game in the filtered library gets a built `Element` every frame.** `list_body` loops over all `shown` games pushing a `context_menu(row(...))` per game, and `grid_body` does the same with `card(...)`; `view()` is called on every frame and there is no `lazy`, `Responsive`, or any viewport-limited child. The only bound on the work is `search()`, which does not reduce the count when no filter is set. | `crates/app/src/view/library.rs:377-397` (`list_body`, the `for game in games` loop at `:379`, the per-row push at `:393-397`), `:436-456` (`grid_body`, loop at `:440`, per-card push at `:443-447`), reached unconditionally from `:358-363`; no `lazy`/`responsive` anywhere in the file (`grep -n "lazy\|Lazy\|responsive" crates/app/src/view/library.rs` → no matches). The decoded-image cull at `iced/tiny_skia/src/engine.rs:585-588` confirms the renderer, not the view, is where off-screen work is dropped. | View construction, layout and diffing are O(library size) **per frame**, on the UI thread, on a CPU renderer. This is also the multiplier that makes PERF-01 and PERF-02 apply to the whole library rather than to the visible page, and it is why the cover cache has no natural bound. | Wrap the body in iced's `lazy` (or a virtualized list) so only the visible rows plus a small overscan are constructed, and drive it from a `(library revision, query, category, sort, scroll offset)` key. That single change removes the O(N)-per-frame term from view construction, from the cover `stat`/`open` (PERF-01), and from the decoded-cover residency set (PERF-02). **Status: FIXED `be31a7b`.** The body is windowed rather than `lazy`-wrapped, and the substitution is deliberate: iced's `List` cannot lay out the wrapping grid this page needs, so the page computes its own `visible_range` from the scroll offset (published by `Scrollable::on_scroll` into `State`) and builds only the rows and grid lines in that range, with fixed-height spacers preserving the height the full list would have had. The window carries two viewports of margin each side, and the pre-first-scroll fallbacks are named constants rather than guesses. Measured against the real binary, 100 forced redraws, CPU time from `/proc/<pid>/stat`: **4,040 ms before, 490 ms after**, with a no-cover floor of 660 → 460 ms — so pre-fix 3,380 of those milliseconds were cover-attributable and post-fix the whole run sits 30 ms above the same floor. Built-element counts from the tests, read off by temporarily relaxing each assertion so its message printed the real number and then reverting: **36 rows and 45 cards out of a 500-game library**, against 500 and 500 pre-fix. **What this fix does not include:** `PERF-06` is untouched and now dominates what remains — a tree-wide `strace -c` still shows ~189,000 `statx`, because `RunnerManager::label` walks the runners directory per shown game per frame. |
| PERF-04 | P2 | **The project's claim that startup performs only two filesystem reads is false, and the comment asserting it sits three lines above the calls that contradict it.** `App::init` runs `Settings::load(None)` and `Library::new(None)`, immediately followed by `state.refresh_plugins(&SystemPluginEnv)` and `state.refresh_installers()`. `refresh_plugins` walks `PATH` once per plugin for five plugins and probes `/.flatpak-info`; `refresh_installers` runs `installer_categories()`, and `runner_choices()` → `installed_protons()`, a `read_dir` over the runners directory plus a metadata read per runner. | The claim: `crates/app/src/main.rs:3653-3655` ("The two loads are the only filesystem work done at startup, and neither is fallible"), with the contradicting calls at `:3656-3668`. The work: `crates/app/src/state.rs:916-919` (`refresh_plugins`), `:948-960` (`refresh_installers`); `crates/core/src/plugins.rs:84-86` (`is_installed` → `env.which`), `:258-278` (`detect_package_manager`, up to five `which` scans plus two `/etc` probes), `:437-500` (`plugin_row`/`plugin_rows`, which reach `detect_package_manager` and `in_flatpak`), `:197-240` (`SystemPluginEnv`: `which_in` at `:207`, `Path::exists` at `:210`, `effective_uid()` reading `/proc/self/status` at `:233-238`). **Measured**: `strace -f -tt -e trace=%file` shows **65,279 file syscalls in the first second** and 2,292 in the second, including **17 `statx` of `/.flatpak-info`** — a path only `in_flatpak()` opens, and `in_flatpak` is reached only from `plugin_rows`/`plugins_intro`/`detect_package_manager`, i.e. only from `refresh_plugins`. By contrast the CLI path is genuinely minimal: `strace -f -c -e trace=%file` on `--list` with the 500-game library shows **18 file syscalls total** (15 `openat`, 1 `statx`, 1 `access`, 1 `execve`). | The direct cost is real but modest (a PATH scan and a handful of probes, negligible beside the ~62,500 icon-theme lookups libcosmic itself performs at startup in my run). The durable cost is that a comment stating an invariant that the code does not hold will defeat the next person who measures startup cost, and this project's own audit history shows that class of defect recurring. | Correct the comment, or make it true by moving `refresh_plugins`/`refresh_installers` onto the existing post-startup `Task` path so the window can paint first. Note the icon-theme scan (62,510 of the 67,350 path-bearing startup syscalls, all under `/usr/share/icons` and the host's Nix store) is libcosmic's, not this app's, and is far smaller inside the Flatpak — worth not attributing to this code in any future write-up. **Status: FIXED `d855015`.** The comment is corrected and now enumerates the real work; the row's third clause, "and the README claim", **has no referent** — `grep` finds no such claim in `README.md`, and none was invented to satisfy the row. Re-measured (`strace -f -c -e trace=%file`, release binary under Wayland): **18** `statx` of `/.flatpak-info`, 6 each of `pacman`/`dnf`/`apt-get`, and **187,490 of 190,438** path-bearing syscalls are libcosmic's icon-theme scan under `/usr/share/icons` — not this code's, and far smaller inside the Flatpak. |
| PERF-05 | P2 | **`search()` re-sorts and re-filters the entire library, and `categories()` rebuilds and re-sorts the category list, on every frame — and both allocate `String`s inside sort comparators.** `view()` calls `page.library.categories()` and `page.library.search(...)` unconditionally, on every frame, not only when a query changes. `search` calls `all(sort)`, which clones the game list and sorts it; the comparators call `a.name.to_lowercase().cmp(&b.name.to_lowercase())`, allocating two `String`s per comparison. `categories()` collects an owned `String` per game and sorts with `to_lowercase()` twice per comparison. | `crates/app/src/view/library.rs:643` and `:646` (both calls, inside `view()`, defined `:642`); `crates/core/src/models.rs:722-738` (`all`: the comparator allocations at `:728`, `:733` and the `sort_by_key(\|a\| a.name.to_lowercase())` at `:735`); `:781-795` (`search`: `query.trim().to_lowercase()` at `:782`, `self.all(sort)` at `:783` and the two `retain` passes at `:785`, `:790-793`); `:799-812` (`categories`: `map(...to_owned())` at `:803`, the `to_lowercase` comparator at `:808`). | For an N-game library this is an O(N log N) sort with O(N log N) heap allocations per frame, plus a full `Vec<String>` rebuild, all to produce a value that changes only when the library, the query, the category or the sort key changes — none of which change during a redraw. The `to_lowercase()`-in-comparator pattern multiplies the cost by roughly the comparison count and is trivially avoidable (sort by a precomputed key, or use `eq_ignore_ascii_case`). | Memoize the resolved rows on `(library revision, query, category, sort)` — the same `State` field `crates/app/src/view/library.rs:253-271` already proposes — and recompute only when one of those inputs changes. Replace the comparator `to_lowercase()` calls with a precomputed lowercase key or a case-insensitive comparison. **Status: FIXED `83dc1c3` — and two of this row's own citations never landed.** `Library` memoises the resolved rows as positions in `games` (`RowMemo`) and the category list on its own (`CategoryMemo`), both invalidated by one `invalidate()` that every mutating method calls; the sorts decorate with precomputed keys, so a fold happens once per game rather than once per comparison. Measured on a 500-game fixture, folds of the sort/filter keys: `all("name")` 3,242 → 500, `search("game 1", "All", "name")` 4,132 → 1,390, `categories()` 4,002 cold / 0 warm, and **100 redraws of an unchanged page 813,400 → 0** (the whole per-frame cost, which is what the row is about). **The citations above are corrected, not merely re-pointed**, and the fault is this row's rather than the fix's: at the audit's own commit `babaeef`, `all` was already at `:722` (its comparators at `:728`, `:733`, `:735`), `search` at `:781` and `categories` at `:799` — so the cited `:625-639`, `:684-698` and `:702-716` pointed into `load_at`'s comment and past `save`, and had never landed. The call-site citation was wrong the same way: `view()`'s two calls are at `library.rs:643` and `:646`, not `:286-291`, which is `WINDOW_VIEWPORTS`' doc comment. This is the third family of citation errors this file records, after `PERF-01`/`PERF-02` and `ARCH-18`, and the same lesson: a `path:line` citation is a checkable claim and this row's were not checked. **The row's own test passed without inspecting what it claimed, and the agent caught it:** the first version of `every_mutation_is_visible_to_the_next_search` warmed one key and re-read through a *different* one, which recomputes whether or not anything was invalidated — dropping `invalidate()` from `remove` left it green. It now reads back through the key it warmed; independently re-proved by the reviewer, who dropped `invalidate()` from `remove` and watched it fail. That mutation fails as a panic (`index out of bounds: the len is 20 but the index is 20`) rather than as an assertion, because the memo holds *positions* — which is also why the invariant is safe: none of `load`, `load_at`, `upsert`, `add`, `remove`, `update` or `mark_played` can shrink `games` without `invalidate()`, and `games` is private with no writer outside `models.rs`. |
| PERF-06 | P2 | **`RunnerManager::label` is uncached, walks the filesystem, and is called once per shown game per frame — and it reads and JSON-parses each runner's metadata only to discard the parsed value.** The code documents this against itself, and the fix, at length. `label()` checks `candidate.exists()` and then constructs a `ProtonRunner` via `discovered()`, whose `new()` eagerly calls `read_family_id` → `read_metadata`, which reads and `serde_json`-parses the runner's `.gamehandler.json`; only `family_label()` and `name()` are used, and the parsed `Value` is dropped. | The self-documenting comment: `crates/app/src/view/library.rs:537-541` ("**This is called from the render path, and that is a known cost** … `RunnerManager::label` is uncached and walks the filesystem. `view` runs every frame, so taking the manager here reintroduces exactly that — once per shown game per frame."). The implementation: `crates/core/src/runners/mod.rs:1176-1193` (`label`), `:651-665` (`ProtonRunner::new`, the eager `read_family_id` at `:657`), `:1034-1070` (`read_metadata`) and `:1072-1080` (`read_family_id`). Call sites: `crates/app/src/view/library.rs:1073-1076` (`row_labels` → `resolved_runner_label`, inside the per-game loop) and `:1004` (`grid_body`, defined `:973`). **Measured**: 3,750 `statx` on `/tmp/gh-perf/data/gamehandler/runners` in the 3.572 s / 100-redraw window (`strace`, same harness as PERF-01). | Each frame performs a `statx` plus a file read plus a JSON parse per row, for a value that changes only when a runner is installed or removed. Note also that `ProtonRunner::new`'s eager metadata read is a regression against the reference's *lazy* `@property` — `read_family_id` is paid even when the caller only wants `name()`, which is exactly what `label()` does. | Resolve the label when the library is re-read and store it in `State` (the fix the comment names); at minimum make `family_id` lazy as the Python was, so a caller that only needs the display name does not read and parse metadata. **Status: FIXED `83dc1c3` — with the fix taken from the second half of the recommendation rather than the first.** The label is memoised against the runner directory's mtime, which is the same `stat` the uncached body already paid as `candidate.exists()`, so the check adds no syscall the render path was not making; what it removes is the directory walk, the file open and the parse behind it. Measured on a 5-runner fixture, 100 frames of 20 rows (2,000 resolutions), `strace -e trace=openat,statx`: **`.gamehandler.json` opens+parses 2,000 → 5**, and statx on the runner directories 4,005 → 2,015 — 401 per frame → 20. The install path writes metadata into the staged tree before renaming it into place, so a build is never visible under its final name without it. This row's citations were **also mostly wrong when it was written**, on the same measurement: at `babaeef` `RunnerManager::label` was at `:1176`, not `:1093-1110`; `ProtonRunner::new` at `:651-665` with its eager `read_family_id` at `:657`, not `:621-644`/`:626-630`; `read_metadata` at `:1034-1070`, not `:971-989`; `read_family_id` at `:1072-1080`, not `:992-1000`; the call sites at `library.rs:1073-1076` (`row_labels`) and `:1004`, not `:383` and `:441`. The one citation that did land, `library.rs:253-271` for the self-documenting comment, is the comment at `:537-541`. All are corrected above. The second half of the recommendation — the eager `read_family_id` that PERF-06's own evidence calls "a regression against the reference's *lazy* `@property`" — is **not** done: the memo removes the cost from the render path, which is what the row measures, but a caller that wants only `name()` still parses metadata. That residual is left open deliberately and the agent's commit says so. Two other code citations to the same function — `view/settings.rs:275` and `view/form.rs:2087`, `:2710`, `:2772`, all naming `runners/mod.rs:1119` for `RunnerManager::choices` — are stale for a *different* reason (the function is at `:1322`) and lie outside this row; they are recorded separately. |
| PERF-07 | — | **The 27 threads are all demand-spawned runtime workers; none polls, and two of them exit on their own.** The question of whether the thread count hides an always-running poller is settled by reading each thread's blocking state. Every one of the 27 was parked in a blocking wait: 3 `gamehandler` (main), 3 `notify-rs inoti` (the filesystem watcher), 1 `smithay-clipboa`, and 1 `tokio-rt-worker` in `do_epoll_wait`; the remaining 19 `tokio-rt-worker` threads in `futex_do_wait`. Two threads exited spontaneously at t+10.03 s, which a fixed always-running pool would not do. | Method: `/proc/<pid>/task/*/stat` field 3 (name) and `/proc/<pid>/task/*/wchan` inspected for every tid of the running app; thread count from `/proc/<pid>/status` `Threads`. Confirmed stable: `Threads` read 27 at t=10 s, 20 s, 30 s, 40 s, 50 s, 60 s and 70 s in one 70 s run, and 27 in every other run. The two exits appear in the timestamped `strace -f` as `+++ exited with 0 +++` at t+10.032 s and t+10.034 s. | None: this is the expected shape of a tokio-based libcosmic app, and it is recorded because "27 threads on an idle app" invites the opposite reading. | No change. If the thread count is ever revisited, the two that exit are the ones to account for, not the 25 that persist. **Status: CLOSED — NOT A DEFECT.** Re-verified at the UX-27 revision — the probe was re-run against a live instance under a headless `sway` (`WLR_BACKENDS=headless`, `wayland-2`): `Threads` read 27 at every one of eight samples across a 70 s idle run, and the per-tid `wchan` census is the same shape — 3 `gamehandler`, 3 `notify-rs inoti`, 1 `smithay-clipboa`, 1 `tokio-rt-worker` in `do_epoll_wait`, 19 `tokio-rt-worker` in `futex_do_wait`, every thread state `S`. No poller appeared; the count the row predicted is the count measured. |
| PERF-08 | — | **Minor RSS drift that is not attributable to a leak on the evidence gathered.** Across a 60 s run with six forced redraws at six *different* window heights, RSS rose from 771,420 kB to 772,492 kB (+1,072 kB) while fd count and thread count were exactly constant. I could not attribute this to any single cache. | `perf-leak.sh` sampling loop; output `t=10s fds=42 threads=27 rss=771420kB` … `t=70s fds=42 threads=27 rss=772492kB`. | If it is the raster cache, the growth is explained by six distinct window sizes each producing a new `(id, width, height)` resampled entry (`iced/tiny_skia/src/raster.rs:192`) — but at ~179 kB per redraw this is not material at any realistic rate, and the same run shows no fd or thread growth at all. | No action needed on this evidence. If a long-session leak is suspected, sample `VmRSS` over hours of *idle* time rather than over forced resizes, since the resize itself changes the cache key set. **Status: CLOSED — NOT A DEFECT.** Re-measured at the UX-27 revision, at a constant size. Under headless `sway` (1280×693, one headless output): an untouched idle run holds `VmRSS` at 64,608 kB for all eight samples of a 70 s window — dead flat, where the row's original measure needed resizes to move. Forced redraws at constant size via `wtype` Tabs at first *appeared* to drift (+1,808 kB over 12 invocations) — a result worth chasing rather than reporting — and the chase identified the stimulus, not the app: each `wtype` call creates a new `zwp_virtual_keyboard_v1`, whose keymap fd/mmap lands in the client, so the growth tracked *keyboard creations*, not redraws. The control settles it: 24 Tabs inside **one** virtual keyboard cost +252 kB total (~10 kB/key, allocator churn), and three further 24-key rounds cost +160/+148/+152 kB each — the ~150 kB per round being the keyboard, with the 24 redraws it carried costing ~0 kB apiece. At constant size there is no measurable per-redraw drift; the row's raster-cache hypothesis for the resize-keyed growth stands, and the flat idle run is the complementary evidence its tail asked for. |
