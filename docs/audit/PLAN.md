...
| `ARCH-05` | A core module's own header declares the live easy-install path dead | S5 | — | Rewrite `installers.rs:15-23` in the present tense naming `easy_install_worker`; verify every function the header calls callerless has a non-test call site. | OPEN |
...
| `ARCH-16` | A comment embeds a grep transcript as its own proof, and the transcript's line numbers no longer land | S5 | — | Re-derive the transcript against the current tree, or replace it with a citation that does not age; verify the cited lines contain what the paragraph says. | OPEN |
