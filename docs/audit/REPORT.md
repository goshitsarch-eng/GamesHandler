...
**This is not a final report.** 10 of 128 defects are still open, most of them
...
them refuted, and this table says 46 defects — and why `PERFORMANCE.md` holds 8
while this table says 6, the pair `PERF-07`/`PERF-08` having been closed as
not-a-defect on re-measurement. Those rows used to be counted in both, which is what made the two totals
...
| P3 | 4 | 2 `UX`, 1 `PKG`, 1 `SEC`. Edge cases, cosmetic divergences, and comments or tests that describe something the code does not do. |
