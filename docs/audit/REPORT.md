...
**This is not a final report.** 55 of 130 defects are still open, most of them
...
| P3 | 42 | Edge cases, cosmetic divergences, and comments or tests that describe something the code does not do. The 17 open `BUG` rows are all here, along with 10 `UX`, 6 `ARCH`, 4 `SEC`, 3 `PKG` and 2 `PERF`. |
