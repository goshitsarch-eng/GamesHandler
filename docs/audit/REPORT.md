...
**This is not a final report.** 51 of 129 defects are still open, most of them
...
|---|---|---|---|---|---|
| `BUG-xx` | Bugs, reliability, feature completeness | 46 | 30 | 2 | 16 |
...
| P2 | 12 | Actionable. The largest concentration is `UX` (6); the rest are 4 `ARCH`, 1 `BUG` and 1 `PKG`. |
| P3 | 39 | Edge cases, cosmetic divergences, and comments or tests that describe something the code does not do. The 15 open `BUG` rows are all here, along with 10 `UX`, 5 `ARCH`, 4 `SEC`, 3 `PKG` and 2 `PERF`. |
