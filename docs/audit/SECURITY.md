...
| SEC-03 | P2 | The Authenticode authenticity decision is weaker than it reads. The publisher test is a case-folded **substring** search over the verifier's whole merged stdout+stderr, not a comparison against a parsed `Subject:` field; and the pinned Microsoft root is passed as `-CAfile`/`-TSA-CAfile` for exactly one of the ten recipes. For the other nine, `microsoft_trust_root` is `false`, no trust anchor is passed, and the decision reduces to "`osslsigncode` exited 0 and printed `Signature verification: ok`" plus "the literal publisher string appears somewhere in the output". | `crates/core/src/installers.rs:1520-1524` (substring over `text.to_lowercase()`); `:1486-1497` (`-CAfile`/`-TSA-CAfile` only under `if installer.microsoft_trust_root`); `:325` (the only recipe with `true`); `:1445` (`AUTHENTICODE_ROOT_NAME`). There is no test counting the recipes that set the flag — only `assert!(ubisoft.microsoft_trust_root)` at `:3580` and the prose claim at `:3571` — so nothing would notice a tenth recipe silently gaining or losing it | If `osslsigncode verify` without `-CAfile` accepts a self-signed certificate, then the check is defeated by a self-signed certificate whose Subject carries the expected publisher string — the signature verifies cryptographically, the exit code is 0, the success line prints, and the substring appears. The download-origin check (`:1347-1371`) still stands in front of it, so exploitability requires control of an allowed host or of the releases JSON; the point is that the publisher test then provides no additional assurance. **The consequence is unverified**: `osslsigncode` is not installed in this environment (`which osslsigncode` → not found), so it was not executed. | Require the pinned root for every recipe — the `.pem` is already installed by the manifest — or parse the `Subject:` line and compare the whole field in order, rather than testing for a substring of the whole output. Settle the unverified half with one run of `osslsigncode verify -in <self-signed PE, Subject CN=Valve Corp.>` and record the exit code. **Status: FIXED `937ef5b` — and this closes the item this file lists as unsettleable.** `verify_installer_authenticity` no longer test-matches the verifier's whole output. The new `subject_values` (`crates/core/src/installers.rs`) collects the value of every `Subject:` line — the signer's, each timestamp signer's and every chain certificate's, all genuine subjects — and the publisher test runs against those and nothing else. The half that determines impact was **measured, not reasoned about**, against the real `osslsigncode` 2.14 inside the Flatpak: without `-CAfile`, a self-signed certificate exits **1** with `Error: self-signed certificate` and prints no `Signature verification: ok` line; with `-CAfile` (Ubisoft's recipe shape) a certificate whose `Subject` is `O=Evil Example Ltd,CN=Totally Unrelated Signer`, signed with `-n "Valve Corp."`, verifies **exit 0**, prints `Signature verification: ok`, and carries `Text description: Valve Corp.` — so the old predicate accepted it, which is the finding's claim confirmed by execution rather than by reading. This is a **deliberate divergence from `installers.py:594-595`** and is recorded as one, per the brief's decision order. Regression-tested by `the_publisher_must_be_the_certificates_not_a_field_the_signer_chose`. **Corrected - two claims in the paragraph above are false.** The publisher test does *not* run against those subjects "and nothing else": a `Subject:`-shaped continuation line inside the signer's own `-n` text is accepted, so the signer still chooses part of the matched input (`SEC-11`). And that named test cannot observe the case - its fixture puts `Text description: {description}` on a single line (`:4387`) behind a fake verifier, so it pins the field-name scoping for the shape it was given, not for the shape that defeats it. The `-CAfile` measurement above stands unchanged. |
...
| SEC-11 | P1 | **The approved-publisher gate is bypassable: signer-chosen text is read as a certificate subject.** `subject_values` (`crates/core/src/installers.rs:1622-1627`) does `line.trim().strip_prefix("Subject:")`, so the prefix test runs **after** trimming and accepts any line that trims to a `Subject:` - including a continuation line of the `-n` / `Text description:` authenticated attribute, which the signer chooses and which the verifier prints verbatim. The predicate is then `publishers.iter().any(\|p\| subjects.iter().any(\|s\| s.contains(&p)))` (`:1597-1605`), a case-folded substring test. Reproduced against real `osslsigncode` 2.14 inside the Flatpak with a certificate whose genuine subject is `C=US, O=Evil Example Ltd, CN=Totally Unrelated Signer`: signing with `-n "$(printf 'Totally Unrelated\n\t\tSubject: C=US,O=Valve Corp.,CN=Valve Corp.')"` makes `verify -CAfile` exit 0 and print a `Subject: C=US,O=Valve Corp.,CN=Valve Corp.` line that exists in no certificate, so `verify_installer_authenticity` returns `Ok(())` for a binary signed by an unrelated publisher. | `crates/core/src/installers.rs:1622-1627` (the forgiving extraction), `:1597-1605` (the predicate), `:430` (the steam recipe's `publishers: &["Valve Corp."]`), `data/microsoft-identity-verification-root-ca-2020.pem` (one broad root, `Microsoft Identity Verification Root Certificate Authority 2020`); the existing test `the_publisher_must_be_the_certificates_not_a_field_the_signer_chose` (`:4371`) cannot see it because its fixture puts `Text description: {description}` on one line (`:4387`) behind a fake verifier, so the multi-line `-n` shape never occurs | Every recipe's gate is a code-execution decision - the recipes gate Steam, Blizzard, Epic, EA, Ubisoft, GOG, Amazon, Rockstar and Discord - so a binary from an unrelated or compromised publisher passes the publisher check. The `-CAfile` chain check and the download-origin check (`:1347-1371`) still stand in front of it, so exploitability requires a certificate reachable from the pinned root or control of an allowed download host; the publisher test itself supplies no assurance | Take the subject from the certificate rather than from the verifier's report: `osslsigncode extract-signature -in <installer> -out <tmp.p7b>` (confirmed working in the sandbox) plus `openssl pkcs7 -inform DER -print_certs -noout` returns only the genuine subject. Keep the chain-validity gate unchanged. The regression test must use a **multi-line** `-n`. **Status: FIXED `88419f5`.** The publisher match no longer reads the verifier's report at all: `osslsigncode extract-signature` pulls the PKCS#7 blob out of the installer and the subject comes from the certificates inside it, so signer-chosen text has no path into the comparison. The chain gate (`exit 0` and `Signature verification: ok`) is unchanged and still checked first; new variants `SignatureUnreadable`, `CertificateReaderMissing` and `CertificateReadFailed` all fail closed, and the `0700` scratch directory is removed by a `Drop` guard on success and on every error path. Regression-tested by `a_signer_chosen_newline_cannot_forge_a_certificate_subject`, which reproduces the real 2.14 output for the multi-line `-n` shape the old fixture could not produce; restoring the old extraction makes it fail with `unwrap_err()` on an `Ok(())` - the forged subject accepted. **One measured divergence from the recommended recipe, and it was the recipe that was wrong:** `openssl pkcs7` has no `-nameopt` (`pkcs7: Unknown option: -nameopt`, OpenSSL 3.5.7) and its default rendering is forward-order with a space after each comma, so the catalog's `CN=GOG  sp. z o.o,O=GOG  sp. z o.o` is **not** a substring of it (measured count 0, against 1 for `openssl x509 -nameopt RFC2253`). Matching the single-call form would therefore have refused a genuinely GOG-signed installer, so the fix runs one `openssl x509 -nameopt RFC2253` per certificate. Verified against real signatures: `SteamSetup.exe` still matches its catalog string, the forged file matches none. Residual, to name rather than hide: the match is now scoped to certificates inside the blob, so a publisher string appearing only in a *chain* certificate `osslsigncode` fetched from its trust store would no longer match - safe in principle, since the signer's own certificate is always in the blob, and measured on the three real signatures obtainable here (Steam, Amazon, GOG). |
...
2. **Whether `osslsigncode verify` without `-CAfile` already rejects a self-signed
   ...
   **SETTLED, and the answer is yes — the certificate is rejected.** The tool is
   not on the host but *is* inside the Flatpak (`flatpak run --command=sh` →
   `osslsigncode 2.14`), which is what made the run possible. Without `-CAfile` a
   self-signed certificate exits **1** with `Error: self-signed certificate` and
   prints no `Signature verification: ok`. **With** `-CAfile` — Ubisoft's recipe
   shape, and the shape this app uses — a certificate whose `Subject` is
   `O=Evil Example Ltd,CN=Totally Unrelated Signer`, signed with `-n "Valve
   Corp."`, verifies **exit 0** and carries `Text description: Valve Corp.`, so
   the old substring predicate over merged output accepted it. That is the
   finding's premise confirmed by execution rather than by reading, and the fix
   is in `SEC-03`'s own row above.
