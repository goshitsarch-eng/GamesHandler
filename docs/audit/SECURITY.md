...
No source file was modified. The six areas examined were, in order: the
`finish-args` grant in `build-aux/flatpak/com.goshapps.GameHandler.json`, external
process launching, URL handling, panics reachable from external data, path
handling, and secrets. Every claim below cites a `file:line`; where a claim could
...
| SEC-09 | P3 | Two of the app's process spawns inherit the launcher's entire environment, and they are the two security-relevant ones: the package-manager install and the Authenticode verifier. Every other spawn replaces the environment first. | `crates/app/src/view/plugins.rs:183-193` (`run_to_completion`: `Command::new(program).args(args)` with no `env_clear`), reached through `crates/core/src/plugins.rs:362-376` to `pkexec`/`sudo`; `crates/core/src/installers.rs:1635-1652` (`run_capturing`, `ProcessCommand::new(program).args(arguments)` with no `env_clear`), called from `:1498`. Every other spawn clears: `crates/app/src/main.rs:2765-2767` (`start_prefix_tool`), `:3134,3152` (`run_installer`), `crates/core/src/installers.rs:2043-2045` (`run_with_env`). | The children see `LD_PRELOAD`, `LD_LIBRARY_PATH`, `http_proxy`, `SUDO_ASKPASS`, `GAMEHANDLER_*` and anything else the invoking shell carried. Reachability is limited: inside the Flatpak, `PATH` is fixed by `manifest:24` and the host environment does not propagate into the sandbox at all; outside it, the environment belongs to the user who launched the app; and `sudo` and `pkexec` sanitise their own environments. So this is a genuine inconsistency with security-relevant call sites, not a demonstrated escalation, and it is reported as such. | `.env_clear().envs(base)` at both sites, as the other three spawns already do. `run_capturing` needs nothing in the child's environment: `osslsigncode` is resolved by `which` in the parent at `installers.rs:1477`. |
