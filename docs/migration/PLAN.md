...
| T-12 | `app`: Installers page | **Architecture** | P-51…P-59. **Reassigned from UX (R-13)**, same shape and same rules as T-11, including the `253498d` re-point to `Shell::view_body`/`Shell::update`. Depends on the file chooser (P-57), which is T-15 and stays with UX — build the page and leave the chooser call behind the existing `Message` variant. |
