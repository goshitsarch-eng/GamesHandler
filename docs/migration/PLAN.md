...
| **R-3** | Wine env construction (`runners.py`) is subtle and pinned by ~60 tests: quote-aware env parsing, `pfx` indirection, the double `apply_launch_options`, bounded stderr tail, `RENAME_NOREPLACE`, and bounded tar extraction (**Rust's `tar` crate has no `data_filter`** — must be hand-rolled). | **High** | Port module-by-module against the existing test vectors; `test_security.py` cases are the gate. |
...
| T-02 | `core::paths` + `core::models` + `core::settings` | Arch | Includes lenient JSON + timestamp normalization (R-8). Python-written fixtures. |
