...
- **#48 is open** — the case next to #47, found by the advocate reviewing its fix
  and reproduced by the lead: `run_stage` never compares its function argument to
  the stage name, so `run_stage cli stage_test` reports `cli` passed and exits 0.
  Filed separately rather than folded into the closed #47 on purpose, since the
  subjects differ and widening a closed item's description past its subject is
  this file's own recurring error. Assigned to Packaging; the removal shape
  (carry the function in `STAGES`) is preferred to the detection shape.
