...
| 51 | `runners.rs` — **`remove_press` had no production caller, so its mutation catch was vacuous.** Defined at `:588`, called only from its own test; the site it was extracted from still built the message inline. M1 (helper body → `"system"`) was CAUGHT — but mutating a function nothing calls changes no production behaviour, so the red proved the test *reads* the helper, not that anything *uses* it. Deletion of the helper and its 44-line test **SURVIVED**. Three doc claims false as a result. The compiler was silent by construction: `#[allow(dead_code)]` on `mod view` (`main.rs:38-39`), not removed until T-09 — **the lint that detects this class is off for the entire window in which the class is generated** | Advocate, reviewing the `382f431` rename; lead reproduced | **CLOSED (`c17c7da`)** — the site was wired (`.on_press(remove_press(row))`), which is the repair the lead ruled for: it makes the M1 catch non-vacuous, turns deletion into `error[E0425]`, and makes all three prose claims true at once. **D-46** records the rule | Architecture re-measured post-fix: M1 CAUGHT, M2 (delete helper) now `error[E0425]` in both targets — the advocate's forward prediction confirmed. **And Architecture self-corrected the overclaim in their own approval**: "non-vacuous *because the helper is wired*" was itself D-40's shape. The honest bound, recorded: the **helper's value** is closed; the **site's** mutation (routing around it inline) still survives; and the page is still not production-reachable, so what the wiring bought is a caller *within the crate* — which is exactly why deletion became a compile error |
